...
# 设置当前进程的 CPU 亲和性，只能在第一个 CPU 核上执行
# 传入的参数是一个集合，包含允许进程执行的核心编号
os.sched_setaffinity(pid, {0})  # 0表示第一个核，{0}表示只允许在第一个核上执行

# 验证设置是否成功
print(f"Process {pid} is running on CPUs: {psutil.Process(pid).cpu_affinity()}")
